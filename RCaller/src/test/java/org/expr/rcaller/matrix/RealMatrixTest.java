...
import org.expr.rcaller.matrix.RealMatrix;
import org.expr.rcaller.matrix.SVD;
...
public class RealMatrixTest {
    ...
    public void testSVD() {
        ...
        SVD svd = rm.getSVD();
        double[][] uMatrix = svd.getU().getData();
        double[][] vMatrix = svd.getV().getData();

        assertEquals(10.09901951, svd.getD()[0], delta);
        assertEquals(0.09901951, svd.getD()[1], delta);
